// ==========================================
// 전역 가상 데이터 및 초기화 (API 연동)
// ==========================================

/**
 * 서버(assets/member/members.json)에서 전체 사용자(직원) 목록을 가져옵니다.
...
/**
 * 서버(assets/member/members.json)에 직원 목록을 저장(수정)합니다.
...
/**
 * 서버(assets/member/allmember.json)에서 전체 출퇴근 기록을 가져옵니다.
...
/**
 * 서버(assets/member/allmember.json)에 출퇴근 기록을 저장(수정)합니다.
        ...
        if (btnDeleteSelected) {
            btnDeleteSelected.addEventListener('click', async () => {
                ...
                if (confirm(`선택한 ${checkedBoxes.length}명의 직원을 정말 삭제하시겠습니까?`)) {
                    ...
                    membersData = membersData.filter(u => {
                        const isAdmin = u.role === 'admin' || u.email === 'admin@test.com';
                        return isAdmin || !emailsToDelete.includes(u.email);
                    });
        ...
        if (btnDeleteAll) {
            btnDeleteAll.addEventListener('click', async () => {
                ...
                if (confirm('정말 모든 직원의 정보를 삭제하시겠습니까?\\n(관리자 계정은 제외됩니다) 이 작업은 되돌릴 수 없습니다.')) {
                    membersData = membersData.filter(u => u.role === 'admin' || u.email === 'admin@test.com');
                    const isSuccess = await saveMembers(membersData);
                    if (isSuccess) {
                        renderAdminTable(membersData);
